...
type SortKey = "total_sales" | "order_count" | "total_qty";
...
export default function ProductRanking({ products }: Props) {
  ...
  const filtered = useMemo(() => {
    const items =
      partFilter === "전체"
        ? products
        : products.filter((p) => p.part === partFilter);
    return [...items].sort((a, b) => b[sortKey] - a[sortKey]).slice(0, MAX_ITEMS);
  }, [products, partFilter, sortKey]);
